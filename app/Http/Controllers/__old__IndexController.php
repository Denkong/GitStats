...
/*
    function CheckAndAdd($arrFiles,$username,$reponame){
      $needCheck=[];
      foreach ($arrFiles as $key => $value) {
        if ($value['size'] === 0) {
          array_push($needCheck, $value['path']);
        }else{
          // array_push($this->reqSend, $value['name']);
          $this->reqSend[$value['path']]=[
            'filename'=>$value['name'],
            'totalCommitChange'=>0,
          ];
        }
      ...
      if ($needCheck){
        foreach ($needCheck as $key => $value) {
          $newclient = new Client();
          $newres = $newclient->get("https://api.github.com/repos/${username}/${reponame}/contents/${value}");
         
            $arrNextFiles=json_decode($newres->getBody()->getContents(), true);
            self::CheckAndAdd($arrNextFiles,$username,$reponame);
    ...
    public function savedata(Request $req)
    {
      ...
      if (empty($req->gitRepo) || empty($req->dateStart) ||empty($req->dateEnd) || preg_match($regular, $req->gitRepo)==0){
        ...
        */
        ...
        $dataEnd=$req->dateEnd;
        // dd("https://api.github.com/repos/${arr[1]}/${arr[2]}/commits?since=${dataStart}&until=${dataEnd}");
       
        /*
        $res = $client->get("https://api.github.com/repos/${arr[1]}/${arr[2]}/contents");
        //  if($newres->getStatusCode()===403){
        //     return redirect('/404');
        //   } else {
        // dd($res->getBody()->getContents());
        $arrFiles=json_decode($res->getBody()->getContents(), true);
        self::CheckAndAdd($arrFiles,$arr[1],$arr[2]);
*/
